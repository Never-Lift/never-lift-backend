...
O backend exige `physicsContractVersion=2.0.0` junto do catálogo `2026.6` e persiste a versão em cada resultado; versões físicas incompatíveis não são comparadas diretamente. Na Parte 2d, a implementação automatizada está concluída e a validação manual permanece pendente. A especificação e as fontes estão em [`docs/contracts/module-2-physics-v2-proposal.md`](docs/contracts/module-2-physics-v2-proposal.md).
