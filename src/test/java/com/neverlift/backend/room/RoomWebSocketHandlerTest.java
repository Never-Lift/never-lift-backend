...
class RoomWebSocketHandlerTest {
    ...
    void broadcastsJoinAndSupportsReadyCancellationAndHostStart() throws Exception {
        ...
        handler.handleMessage(session, new org.springframework.web.socket.TextMessage("""
                {"type":"join_room","payload":{"roomCode":"1234","trackCatalogVersion":"2026.12","physicsContractVersion":"2.0.2"}}
                """));
        handler.handleMessage(session, new org.springframework.web.socket.TextMessage("""
    ...
    private RoomResponse room(String code, UUID hostId) {
        return new RoomResponse(code, "Test", hostId, "Host", "albert-park", "2026.12", "2.0.2", 1, 22,
                new RoomResponse.RoomResponseSettings("albert-park", "2026.12", "2.0.2", 22,
