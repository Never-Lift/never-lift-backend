...
- O online exige conta registrada. Guest pode visualizar a tela de entrada escurecida/desfocada, mas não lista, cria nem entra em salas.
- Não existe senha de sala. Sala pública aceita entrada direta pela listagem; sala privada não aparece nela e usa apenas o código numérico de quatro dígitos como segredo.
- A criação solicita somente nome opcional e visibilidade. Pista, limite, bots e dificuldade são ajustados ao vivo pelo host dentro da sala; não há botão de salvar.
- O host não possui estado de pronto. Somente os demais humanos confirmam ou retiram o pronto; o host inicia a classificação quando todos eles estiverem prontos.
