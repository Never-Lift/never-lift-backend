...
| 2 — Suporte a corrida local | catálogo `2026.5` validado manualmente em 24/08/2026; simplificação #72 para F1 único concluída no contrato histórico `1.3.0`; Parte 2d (contrato físico v2, faces canônicas de barreira, versionamento de resultado e remoção de boost): implementação automatizada concluída; revisão de fidelidade das 24 pistas no catálogo `2026.9` concluída por automação, com zebras autorais por curva, pits/estruturas/cercas métricos, prevenção de sobreposições, cruzamento de Suzuka e via visual de escape do Rettifilo aguardando validação manual integrada; Parte 2d e Módulo 2 ainda não estão prontos |
