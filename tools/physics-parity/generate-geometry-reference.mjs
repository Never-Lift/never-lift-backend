...
try {
  ...
  for (const item of catalog.tracks) {
    ...
    for (const distance of sampleDistances) for (const offset of [0, 8, -15]) {
      ...
      Object.assign(car,{position,trackDistanceMeters:distance,angle:Math.atan2(tangent.y,tangent.x),velocity:{x:tangent.x*30,y:tangent.y*30},surface:geometry.getSurfaceAt(position,distance)})
  ...
  const sources = {}
  for (const name of ['src/race/TrackGeometry.ts','src/race/RaceEngine.ts','src/race/constants.ts']) sources[name]=createHash('sha256').update((await readFile(resolve(frontend,name),'utf8')).replaceAll('\r\n','\n')).digest('hex')
  const output=resolve(root,'src/test/resources/physics/typescript-geometry-2.0.2.json')
  const result=JSON.stringify({physicsContractVersion:'2.0.2',sources,tracks},null,2)+'\n'
  if(process.argv.includes('--check')) { if(await readFile(output,'utf8')!==result)throw new Error('Stale geometry reference') }
